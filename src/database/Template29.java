...
public class Template29 
{
    ...
    public Template29(String tableName, ArrayList colData, Properties prop, boolean option) 
    {
    ...
    public void execute()
    {
        ...
        {
            System.out.println("Class database.Template29 error: " + ex);
        }
        ...
        try 
        {
            ...
            for(int i = 42; i < colData.size(); i = i + 2)
            {
                ...
                {
                    stmt.setInt(43, java.sql.Types.INTEGER);//upaction
                    stmt.setLong(44, java.sql.Types.BIGINT);//entrytype
                }
                else
                {
                    stmt.setString(43, colData.get(i + 0).toString().trim());
                    stmt.setFloat(44, Float.parseFloat(colData.get(i + 1).toString().trim()));
                }
